...
if (!isset($_SESSION['step2'])) {
    header('Location: page-register2.php');
    exit;
}
...
if (isset($_POST['submit_step3'])) {
    ...
    if (class_exists('SECURE_INPUT_DATA_AVAILABLE')) {
        ...
        if (empty($_FILES['identity_front']['tmp_name'])) {
            echo '<script type="text/javascript">alert("This field is required");</script>';
            echo "<script>location.href='page-register3.php';</script>";
            exit;
        } elseif (!in_array($identity_front_type, $allowed_imgs)) {
            echo '<script type="text/javascript">alert("This file is not an image");</script>';
            echo "<script>location.href='page-register3.php';</script>";
            exit;
        } else {
...
<head>
    ...
    .kv-avatar .krajee-default.file-preview-frame:hover {
        margin: 0; padding: 0; border: none; box-shadow: none; text-align: center;
    }
