...
$params = array(
    ...
    // LIVE keys (from environment)
    "private_live_key" => getenv('STRIPE_LIVE_SECRET'),
    "public_live_key"  => getenv('STRIPE_LIVE_PUBLIC'),

    // TEST keys (from environment)
    "private_test_key" => getenv('STRIPE_TEST_SECRET'),
    "public_test_key"  => getenv('STRIPE_TEST_PUBLIC')
);

// 🔐 Safety check: stop execution if keys are missing
if (
    ($params['testmode'] === "on" && (!$params['private_test_key'] || !$params['public_test_key'])) ||
    ($params['testmode'] !== "on" && (!$params['private_live_key'] || !$params['public_live_key']))
) {
    die("Stripe keys are not configured properly.");
}
